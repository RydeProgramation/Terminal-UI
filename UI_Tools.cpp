...
bool UITools::IsOutSide(Coordinate<int> xy, int BorderWidth)
{
...
bool UITools::IsOutSide(Coordinate<int> xy)
{
...
void UITools::MoveCursorTo(Coordinate<int> Coordinate, int BorderWidth)
{
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	COORD position = { static_cast<SHORT>(Coordinate.GetX().GetDataActual() + (BorderWidth * 2)), static_cast<SHORT>(Coordinate.GetY().GetDataActual() + BorderWidth) };
	SetConsoleCursorPosition(hConsole, position);
}

void UITools::MoveCursorTo(Coordinate<int> Coordinate)
{
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	COORD position = { static_cast<SHORT>(Coordinate.GetX().GetDataActual()), static_cast<SHORT>(Coordinate.GetY().GetDataActual()) };
	SetConsoleCursorPosition(hConsole, position);
}
...
bool UITools::IsInWidget(Widget* WIDG, Coordinate<int> crd)
{
...
Size<int> UITools::GetConsoleSize(int BorderWitdh)
{
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	...
	return Size<int>(Conlumns, Rows);
}

Size<int> UITools::GetConsoleSize()
{
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	CONSOLE_SCREEN_BUFFER_INFO csbi;
