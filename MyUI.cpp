...
void MyUI::Start()
{
	...
	CreateWidgetWait(new Widget(/*100*/0, -1, 22, 5, MiddleCenter, hello_, "Hello"));

	/*for (int i = 0; i < 100; i++)
	{
		Widgets[WidgetResearch("Hello")]->AddToPosition(-1, 0);
	...
	ToDestroyWidget("Hello");

	CreateWidgetWait(new Widget(0, -1, 52, 5, MiddleCenter, hello2_, "Hello2"));
	...
	//ToDestroyWidget("Hello2");

	//CreateWidgetWait(new Widget(0, -1, 54, 5, MiddleCenter, hello3_, "Hello3"));

	//Widgets[WidgetResearch("Hello3")]->SetColor(11);
/*
	...
	for (int i = 0; i < 10; i++)
	{
		//Widgets[WidgetResearch("Hello3")]->AddToPosition(0, 1);
	...
	for (int i = 0; i < 5; i++)
	{
		//Widgets[WidgetResearch("Hello3")]->AddToSize(0, -1);
		//Widgets[WidgetResearch("Hello3")]->AddToColor(1);

		Sleep(50);
	}*/

	// ToDestroyWidget("Hello3");
	...
	GetConsoleSize(&cy, &cx, false);

	cx = cx / 2 - (120 / 2);
	cy = cy / 2 - 6;

	CreateWidget(cx, cy, 120, 6, parag, "Paragraphe");
...
void MyUI::CreateWidgetWait(Widget* WIDG)
{
	CreateWidget(WIDG);

	while (GetWidget(WidgetResearch(WIDG->GetName().GetDataActual()).value()).GetChange().GetDataActual()) {}
}
